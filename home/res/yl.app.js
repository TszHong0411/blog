window.YLApp = {
   ...
   * @param data 數據
   * @param target 發送目標，可以是單個id，id數組，或者true（發送給所有窗口）
   * */
  ...
  onReady: function (cb) {
    if (this._cbReady === false) return; //只允許ready一次
    if (!cb) {
      cb = function () {
      }
    }
  ...
  switch (msg.type) {
    case "ylui-ping":
      if (YLApp.id) {
        ...
        if (YLApp._cbReady) {
          var relCbReady = YLApp._cbReady(); //執行ready的回調
          YLApp._cbReady = false;//清空ready
          if (relCbReady !== false)
            YLApp.emit('ready', null, true); //發送ready事件
        }
        if (!YLApp._created) {

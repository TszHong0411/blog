  ...
  YL.vue = new Vue({
    ...
    methods: {
      ...
      initRuntime: function (first) {
          ...
          this.$nextTick(function () {
            ...
            for (var i = 0; i < box.length; i++) {
              ...
              $(e).niceScroll({
                ...
                cursorborder: "none", // CSS方式定義滾動條邊框
            ...
            $("#yl").css({ 'opacity': 1, display: "block" }); //顯示
          });
        }
        ...
        if (first) {
          setInterval(function () {
            //wallpaperSlided:Date.now(), //記錄最近一次切換壁紙的時刻
            if (!that.configs.wallpaperSlide) return; //沒開啟則返回
            var itv = that.configs.wallpaperSlideItv * 60 * 1000; //間隔（毫秒）
            var last = that.configs.wallpaperSlideTime;//記錄最近一次切換壁紙的時刻
            var indexLast = that.configs.wallpaperSlideIndex;//記錄最近一次切換壁紙的index
            var now = Date.now();
            if (now > (itv + last)) {
              that.configs.wallpaperSlideTime = now; //記錄當前時間
              var wallpapers = that.configs.wallpapers;
              var len = wallpapers.length;
              var index = 0;
              if (that.configs.wallpaperSlideRandom) {
                index = YL.util.randInt(0, len - 1);//隨機取下標
              } else {
                //順序取下標
                index = indexLast + 1;
                if (index >= len) {
              ...
              if (wallpapers[index]) {
                that.configs.wallpaper = wallpapers[index].image;//切換
      ...
      appOpen: function (id, options, source) {
        ...
        //隱藏菜單和操作中心
        // appMerge.plugin || this.runtime.winOpened++;//計數
        this.runtime.winOpenCounter++;//計數
        var dx = parseInt(this.runtime.winOpenCounter % 10) * this.runtime.clientSize.height * 0.01;//坐標偏移量
        //該函數支持字符串轉數字，其中x和y將被替換為屏幕的寬和高
        ...
        var win = {
          ...
          data: {},//data域，使用YLApp時傳遞給子頁面
          pong: null, //最後pong的時間
          childSupport: false,//子頁支持可用
          init: true,//init狀態，用於顯示封面
        };
        win = Yuri2.jsonMerge(win, appMerge);
        win.style = Yuri2.jsonDeepCopy(win.style);//深拷貝對象
        ...
        that.$nextTick(function () {
          ...
          YL.util.iframeOnClick.track(ifr, function () {
            if (Object.getOwnPropertyNames(YL.util._iframe_click_lock_children).length === 0) {
              ...
              ContextMenu._removeContextMenu();//關閉右鍵菜單
            }
          }, winID);
      ...
      winTitleMouseDown: function (id, e) {
        //忽略右鍵
        if (e.which === 3) return;
        var isMouse = !e.changedTouches;//是否是鼠標事件，false為觸屏；
        ...
        var fnMouseMove = function (e) {
          ...
          var x = p.x;
          var y = p.y;
          if (win.state === 'max' && (y - win.drag.y > 2)) that.winRestore(id);//在全屏狀態拖動，自動還原
      ...
      winContextMenu: function (id, e) {
        ...
        menu.push([YL.util.getStrFa("magic") + YL.lang("CollectAsApplication"), function () {
          layer.prompt({
            ...
            YL.addApp(appId, app);
            YL.addShortcut(appId);//自動添加到桌面
            YL.addMenuItem(appId);//自動添加到菜單
            YL.util.simpleMsg(YL.lang("Added"));
          });
        }, !YL.static.changeable]);
        menu.push('|');
      ...
      shortcutOpen: function (s, id) {
        ...
        if (!s || s.drag.moved > 20 || now - this.runtime.shortcutOpenedAt < 500) {
          //忽略拖動產生的click和touch+click導致的短時間多次打開
          return;
        }
        if (s.children) {
          //打開抽屜
      ...
      shortcutMouseDown: function (id, pid, e) {
        if (e.which && e.which !== 1) return; //只響應左鍵
        var isMouse = !e.changedTouches;//是否是鼠標事件，false為觸屏；
        var downTime = Date.now();
        var fnGetPagePointFromEvent = function (event) {
          var p = {
        ...
        var fnMouseUp = function (e) {
          ...
          if (s.drag.moved < 20 && Date.now() - downTime < 300) {
            that.shortcutOpen(s, id); //TODO 此處是否需要加延時操作
          }
          s.drag.moved = 0;
    ...
    watch: {
      ...
      "startMenu.open": {
        handler: function (val, oldVal) {
          //修覆磁貼在全屏切換時布局混亂的bug,人為發出一個resize事件
          if (val) {

...
Array.unique = function (arr) {
  ...
  for (var i = 0; oldArray.length > 0; i++) {
    //要一直把oldArray pop完為止.所以長度會一直變短。所以不能用i < oldArray.length的形式來判斷是否完成.
    newArray.push(oldArray.shift()); //oldArray從最前面開始移出數組元素，這樣新數組的順序不會變。
    for (var j = 0; j < oldArray.length; j++) {
      if (newArray[i] === oldArray[j]) {
        oldArray.splice(j, 1);//刪除重覆的元素
        j--;
      }
    }
  }
...
var Yuri2 = {
  ...
  heredoc: function (fn) {
    //var tmpl = heredoc(function(){/* abcdefg  */})  可以優雅的解決多行文本問題
  ...
  parseURL: function (url) {
    ...
    a.href = url;
    a.href = a.href; //神奇的代碼，借助a標簽把相對路徑轉換為絕對路徑
    return {
      source: url,

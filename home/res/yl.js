window.YL = {
  info: {
    ...
    copyrightDetail: 'YLUI已授權本產品使用。YLUI禁止任何未經作者授權的使用、修改、二次發布，包括從本產品中剝離YLUI獨立使用等侵權行為，違者將追究法律責任。',
    otherStatements: '',
  },
  data: {},
  lang: function (key) {
  ...
  _baseData: function () {
    return {
      // YL:YL,
      configs: {
        topTaskBar: true, //頂部任務欄？默認底部
        sound: false,//開啟聲音
        wallpaper: 'https://cdn.jsdelivr.net/gh/tszhong0411/image/background-2.0.gif',
        wallpaperBlur: false, //壁紙模糊（影響性能）
        wallpaperSlide: false, //壁紙幻燈片
        wallpaperSlideRandom: false, //壁紙幻燈片順序隨機（否則順序播放）
        wallpaperSlideItv: 0.1, //壁紙幻燈片播放切換間隔（單位：分鐘）
        wallpaperSlideTime: Date.now(), //記錄最近一次切換壁紙的時刻
        wallpaperSlideIndex: 0, //記錄最近一次切換壁紙的index
        openMax: 9,//最多開啟N個app
        idCounter: 0,//ID的下標起始
        themeColor: "black",
        autoThemeColor: true,
      ...
      runtime: {
        ...
        lang: 'en',
        winActive: null, //激活的窗口號
        winActiveTime: Date.now(), //上一次激活的時間戳（惰性檢查）
        drag: false, //窗口拖拽中
        time: '', //時鐘
        winOpenCounter: 0, //總計窗口數
        winOpened: 0, //目前打開的窗口數
        data: {}, //數據
        shortcutsShow: true,//圖標刷新動畫輔助
        pluginIconsOpen: false,//插件小圖標打開/關閉
        CalendarOpen: false,//日歷打開/關閉
        tileSize: 0,//磁貼的尺寸（動態計算）
        tilesWidth: 0,//磁貼框的尺寸（動態計算）
        tilesGroupNum: 1,//磁貼框每一行最多幾個組（計算得出）
        wallpaper: '',
        wallpaperScale: 1, //壁紙寬高比
        shortcutNewParamName: "",//圖標的新參數input name
        shortcutNewParamValue: "",//圖標的新參數input val
        isIE: false,//是否IE,
        menuItemCut: null,//菜單項的剪切板
        tileMoved: false, //記錄磁貼移動狀態，防止移動觸發點擊
        customTileRandomToken: YL.util.randInt(1000, 9999),//自定義磁貼iframe的隨機token
        shortcutsGrid: { x: 0, y: 0 },//桌面圖標網格的寬高(int)
        shortcutInsert: null,//即將插入在前的圖標（視覺效果）
        shortcutOver: null,//即將拖動覆蓋的圖標（視覺效果）
        menuOnLeft: true,//移動端：菜單在左（反之在右）
        shortcutWidth: 0,//移動端響應式的圖標尺寸
        shortcutHeight: 0,//移動端響應式的圖標尺寸
  ...
  msg: function (title, content) {
    ...
    this.vue.center.open || this.vue.center.unread++; //未讀標記+1
    this.vue.center.msgNum++; //總數+1
    var indexs = {};
  ...
  addTileGroup: function (groupTitle) {
    ...
    v.tiles.push({ title: '分組' + title, data: [] });
  ...
  loadRes: function () {
    //資源加載
    var that = this;
    var pathRes = '/home/res';
    var counterLoad = 0, maxLoad = 24; //資源加載計數器
    var onLoading = "...";
  ...
  util: {
    ...
    imgUrlToThemeColor: function (url, cb, light) {
      ...
      var img = new Image;
      img.src = url;
      img.crossOrigin = 'anonymous';//跨域聲明（只在chrome和firefox有效——嗎？）
      img.onload = function () {
        try {
          ...
          var data = ctxt.getImageData(0, 0, img.width, img.height).data;//讀取整張圖片的像素。
          var r = 0, g = 0, b = 0, a = 0;
...
YL.util.onloadSafe(function () {
  YL.util.loadScript('/home/configs.js', function () {
    ...
    YL.util.loadContentFromUrl('/home/langs/' + YL.static.lang + '.json', 'GET', function (err, text) {
      if (!err) {
